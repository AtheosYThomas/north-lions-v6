    ...
    <div class="bg-white shadow">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
          ...
          <h1 class="text-2xl font-bold text-gray-900 tracking-tight">編輯系統公告</h1>
        </div>
        <div>
...
const submit = async (status: string) => {
  ...
  try {
    ...
    if (selectedFiles.value.length > 0) {
      ...
      form.value.content.attachments = [
         ...form.value.content.attachments,
         ...uploadedFiles
      ];
    }
    ...
    await announcementsStore.updateAnnouncement(id, form.value);
...
onMounted(async () => {
  const id = route.params.id as string;
  if (!id) return router.push('/announcements');
  
  const existing = await announcementsStore.fetchAnnouncementById(id);
  if (!existing) {
    alert('找不到此公告');
  ...
  form.value.content.body = existing.content.body;
  form.value.content.summary = existing.content.summary;
  if (existing.content.attachments) {
    form.value.content.attachments = existing.content.attachments as any;
  }
  form.value.status.status = existing.status.status as any;
  form.value.settings.isPinned = existing.settings?.isPinned || false;
  form.value.enableAutoPush = existing.enableAutoPush || false;
});
</script>
